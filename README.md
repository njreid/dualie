...
| Feature | Description |
|---------|-------------|
| **Consistent remapping everywhere** | The same config applies to the hardware keyboard (via firmware) AND to the built-in keyboard and any directly-connected keyboards (via the daemon intercept layer) — one config, uniform muscle memory on every machine |
| **Key remapping** | Per-output remap tables — remap any key, optionally requiring/replacing modifier bytes |
| **Modifier remapping** | Transform modifier keys on every keystroke per output (e.g. swap Ctrl↔Alt between machines) |
| **Caps-lock layer** | Hold Caps Lock to turn any key into a chord, virtual action, or desk-jump — works on all keyboards, not just the hardware one |
| **Desk jumps** | Caps-layer entries that immediately switch to output A, output B, or toggle |
| **Virtual actions** | Up to 32 slots that trigger app-launch or shell commands; fired via a dedicated CDC-ACM serial channel — no repurposed HID keycodes |
| **KDL config** | Human-readable `~/.config/dualie/config.kdl`; same format as Niri; hot-reloaded without restart |
| **TUI** | `dualie-tui` — Ratatui terminal UI for editing config, viewing status, and managing sync pairs; no browser needed |
| **One-command firmware flash** | `just flash` reboots the near RP2040 into bootloader via serial and copies the uf2; the near board auto-flashes the far board over UART |
| **Clipboard + file sync** _(planned)_ | Daemon-to-daemon messaging over CDC-ACM serial; no IP stack, no cloud, no third-party service |

See [HARDWARE.md](HARDWARE.md) for the full hardware design.
...
```shell
# Prerequisites: Rust toolchain, just, cmake + arm-none-eabi toolchain (for firmware)

just build          # firmware + all Rust crates
just daemon-build   # cargo build --release (daemon + tui + proto)
just dev            # cargo watch on daemon, for daemon development
just flash          # reboot RP2040-A into bootloader via serial, copy uf2;
                    # RP2040-A then auto-flashes RP2040-B over UART
just install        # install dualie as a user service (launchd on macOS, systemd on Linux)
                    # + install udev rules (Linux) or Karabiner driver prompt (macOS)
```

Run the TUI after the daemon is installed:
```shell
dualie-tui          # connects to ~/.local/run/dualie/daemon.sock for live status
```

#### macOS prerequisites

**Karabiner-VirtualHIDDevice** (the driver component only — Karabiner-Elements app not needed):
```shell
# just install will prompt you; or manually:
# https://github.com/pqrs-org/Karabiner-VirtualHIDDevice/releases
# Approve in: System Settings → Privacy & Security → General
```

The daemon uses this driver to re-inject remapped events at the kernel level. CGEventTap alone
cannot reliably treat Caps Lock as a layer modifier — the OS intercepts it before events reach
the tap. The Karabiner driver solves this. Once approved, it persists across reboots and is
fully independent of the Karabiner-Elements app.

**Accessibility permission**: System Settings → Privacy & Security → Accessibility → add `dualie`.

#### Linux (Wayland / Niri) prerequisites

The daemon grabs keyboards exclusively via evdev and re-injects via uinput. `just install`
sets up the udev rule automatically. To do it manually:

```shell
sudo usermod -aG input $USER   # log out and back in, or use newgrp input
```

---

## Original DeskHop features
...
The project bundles a minimal pico-sdk and TinyUSB to avoid version-mismatch issues.
...
With `just` installed, the easiest path is:

```shell
just firmware-build   # runs cmake + make, outputs build/dualie_board_*.uf2
```
...
An alternative is Docker:

```shell
docker-compose -f misc/docker.yml run --rm build_container
```

To rebuild the ramdisk image, see `disk/` and run `./create.sh` (requires **dosfstools**).
...
- There is no copy-paste or _any_ information sharing between systems in the current release. Planned clipboard and file sync will travel exclusively over the dedicated CDC-ACM serial link between each machine's daemon and its RP2040 — a character device that only the daemon holds open, with no IP stack and no possibility of another process routing through it.
...
- There is no bluetooth or wifi. The hub communicates exclusively over USB CDC-ECM (wired, physical-access-required). No internet access, no cloud.
