...
/* Apply all configured key remaps to an outgoing keyboard report.
 * Remaps are checked in order; only the first matching entry per keycode fires.
 * Called after hotkey detection so hotkeys always use physical keycodes. */
void apply_key_remaps(hid_keyboard_report_t *report, device_t *state) {
    const key_remap_t *remaps = state->config.key_remaps;

    /* Bit-select the current active output (bit 0 = OUTPUT_A, bit 1 = OUTPUT_B) */
    uint8_t output_bit = 1u << state->active_output;

    for (int i = 0; i < MAX_KEY_REMAPS; i++) {
        const key_remap_t *r = &remaps[i];

        /* Skip if this remap doesn't apply to the current output */
        if (!(r->output_mask & output_bit))
            continue;

        /* ── Modifier-only remap (REMAP_FLAG_MOD_ONLY) ──────────────────────
         * Transform modifier bits in-place without requiring a specific keycode.
         * src_modifier bits that are all present get cleared and dst_modifier
         * bits are set.  Allows e.g. LCtrl → LAlt across all keypresses. */
        if (r->flags & REMAP_FLAG_MOD_ONLY) {
            if (r->src_modifier != 0
                    && (report->modifier & r->src_modifier) == r->src_modifier) {
                report->modifier = (report->modifier & ~r->src_modifier) | r->dst_modifier;
            }
            continue;
        }

        /* src_keycode == 0 marks an unused regular-remap slot */
        if (r->src_keycode == 0)
            continue;

        /* All required src_modifier bits must be present (0 = don't care) */
        if (r->src_modifier != 0 && (report->modifier & r->src_modifier) != r->src_modifier)
            continue;

        /* Scan the keycode array for a match and replace in-place */
        for (int j = 0; j < KEYS_IN_USB_REPORT; j++) {
            if (report->keycode[j] == r->src_keycode) {
                report->keycode[j] = r->dst_keycode;

                /* 0xFF = preserve original modifier; otherwise replace */
                if (r->dst_modifier != 0xFF)
                    report->modifier = (report->modifier & ~r->src_modifier) | r->dst_modifier;

                break; // one replacement per remap entry
            }
        }
    }
}
...
/* Send a CapsLock tap to toggle the OS CapsLock state.
 * We queue a key-down then immediate key-up outside normal flow. */
void caps_send_lock_toggle(device_t *state) {
...
/* Apply the caps layer to an incoming report while CapsLock is held.
 *
 * Returns true  if the report was fully handled (caller should NOT send it).
 * Returns false if the report should fall through to normal processing.
 *
 * Key-by-key transform:
 *   1. CapsLock itself is always consumed (never forwarded while held).
 *   2. Escape with caps held → send CapsLock toggle, consume report.
 *   3. For each keycode in the report, look up in the active output's caps
 *      layer table:
 *        CHORD   – replace modifier+keycodes in-place
 *        VIRTUAL – replace the keycode with the vkey slot code
 *        no match – leave as-is (passthrough) or zero it out (swallow)
 */
bool process_caps_layer(hid_keyboard_report_t *report, device_t *state) {
    const uint8_t output  = state->active_output;
    const caps_layer_t *layer = &state->config.caps_layer[output];
    const uint8_t output_bit  = 1u << output;

    /* Step 1: strip CapsLock from the report; it's our modal trigger */
    for (int j = 0; j < KEYS_IN_USB_REPORT; j++) {
        if (report->keycode[j] == HID_KEY_CAPS_LOCK)
            report->keycode[j] = 0;
    }

    /* Step 2: Caps + Escape → toggle OS CapsLock, consume */
    if (key_in_report(HID_KEY_ESCAPE, report)) {
        caps_send_lock_toggle(state);
        return true;
    }

    /* Nothing else pressed alongside caps → consume silently */
    bool any_key = false;
    for (int j = 0; j < KEYS_IN_USB_REPORT; j++) {
    ...
    /* Step 3: transform each keycode through the layer table */
    ...
    for (int j = 0; j < KEYS_IN_USB_REPORT; j++) {
        ...
        for (int i = 0; i < CAPS_LAYER_MAX_ENTRIES; i++) {
            ...
            if (!e->src_keycode)      continue; // unused slot
            if (e->src_keycode != src) continue;
            if (!(e->output_mask & output_bit)) continue;

            if (e->entry_type == CAPS_ENTRY_CHORD) {
                /* Build a new report from the chord definition.
                 * We do this only once and then send it, even if multiple
                 * chord entries fire (unlikely; first match wins). */
                if (!chord_applied) {
            ...
            } else if (e->entry_type == CAPS_ENTRY_JUMP_B) {
                set_active_output(state, 1);
                return true;
            } else if (e->entry_type == CAPS_ENTRY_SWAP) {
        ...
        if (!matched) {
            /* Key has no caps layer entry */
            if (!layer->unmapped_passthrough)
                report->keycode[j] = 0; // swallow
            /* else leave keycode as-is (passthrough) */
        }
    }

    return false; /* caller should still send the (possibly transformed) report */
}
...
void send_key(hid_keyboard_report_t *report, device_t *state) {
    ...
    /* Apply key remaps to the outgoing combined report */
    apply_key_remaps(&combined_report, state);
...
void process_keyboard_report(uint8_t *raw_report, int length, uint8_t itf, hid_interface_t *iface) {
    ...
    /* Track CapsLock physical hold state.
     * We treat CapsLock as a modal layer key rather than a toggle:
     *   - pressed  → caps_held = true  (key NOT forwarded)
     *   - released → caps_held = false */
    bool caps_in_report = key_in_report(HID_KEY_CAPS_LOCK, &new_report);
    state->caps_held = caps_in_report;

    /* Update the keyboard state for this device */
    update_kbd_state(state, &new_report, itf);

    /* While caps is held: route through the modal layer.
     * Hotkeys intentionally bypass the caps layer so firmware combos
     * (switch output, firmware upgrade, etc.) still work physically. */
    if (state->caps_held) {
        bool fully_consumed = process_caps_layer(&new_report, state);
        if (fully_consumed)
            return;
    ...
    /* Normal path: check hotkeys first */
