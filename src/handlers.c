...
/* Receive a single remap entry sent by the other board and persist it locally.
 *
 * Packet data layout (8 bytes):
 *   [0] index        – slot in key_remaps[] (0 .. MAX_KEY_REMAPS-1)
 *   [1] src_keycode  – HID keycode to match
 *   [2] dst_keycode  – HID keycode to emit
 *   [3] src_modifier – required modifier bits (0 = any)
 *   [4] dst_modifier – output modifier bits (0xFF = keep original)
 *   [5] output_mask  – bit 0 = OUTPUT_A, bit 1 = OUTPUT_B
 *   [6] flags        – reserved
 *   [7] padding
 */
void handle_key_remap_msg(uart_packet_t *packet, device_t *state) {
    uint8_t index = packet->data[0];

    if (index >= MAX_KEY_REMAPS)
        return;

    key_remap_t *r  = &state->config.key_remaps[index];
    r->src_keycode  = packet->data[1];
    r->dst_keycode  = packet->data[2];
    r->src_modifier = packet->data[3];
    r->dst_modifier = packet->data[4];
    r->output_mask  = packet->data[5];
    r->flags        = packet->data[6];

    save_config(state);
}

/* Send one remap slot to the other board */
void sync_key_remap(uint8_t index, device_t *state) {
    if (index >= MAX_KEY_REMAPS)
        return;

    const key_remap_t *r = &state->config.key_remaps[index];
    ...
    queue_packet(data, KEY_REMAP_MSG, sizeof(data));
}

/* Push all remap slots to the other board (e.g. after a config change) */
void sync_all_key_remaps(device_t *state) {
    for (uint8_t i = 0; i < MAX_KEY_REMAPS; i++)
        sync_key_remap(i, state);
}
