...
uint16_t tud_hid_get_report_cb(uint8_t instance,
    ...
    /* STATUS report: tell the browser which output is currently active */
    if (instance == ITF_NUM_HID_VENDOR && report_id == REPORT_ID_DUALIE_STATUS) {
        if (request_len >= 1) {
            buffer[0] = (uint8_t)global_state.active_output;
            return 1;
        }
    }
    return 0;
...
/* ── Dualie WebHID config chunk handler ──────────────────────────────────────
 * Called from tud_hid_set_report_cb when a full config blob has been received.
 * Validates the magic, version and checksum, then writes the blob to flash and
 * reloads the running config.
 */
static void apply_hid_config(device_t *state) {
    config_t *rx = (config_t *)state->hid_config_rx;

    /* Basic sanity checks – reject obviously corrupt data */
    if (rx->magic_header != 0xB00B1E5) return;
    if (rx->version     != CURRENT_CONFIG_VERSION) return;

    /* CRC check: daemon truncates calc_crc32() result to uint8_t before storing */
    uint8_t expected = (uint8_t)calc_crc32(state->hid_config_rx,
                                           sizeof(config_t) - sizeof(uint32_t));
    if ((uint8_t)rx->checksum != expected) return;

    /* Apply: copy into running config and persist to flash */
    memcpy(&state->config, rx, sizeof(config_t));
    save_config(state);

    /* Reset receive state */
    state->hid_config_rx_bytes = 0;
}
...
void tud_hid_set_report_cb(uint8_t instance,
    ...
    /* ── Dualie WebHID reports ───────────────────────────────────────────── */
    if (instance == ITF_NUM_HID_VENDOR) {

        /* 0x10: switch active output */
        if (report_id == REPORT_ID_DUALIE_SWITCH && bufsize >= 1) {
            uint8_t idx = buffer[0] & 1;  // clamp to 0 or 1
            set_active_output(&global_state, idx);
            /* Send a STATUS input report back so the browser can confirm */
            tud_hid_n_report(ITF_NUM_HID_VENDOR, REPORT_ID_DUALIE_STATUS,
                             &idx, 1);
            return;
        }

        /* 0x11: config chunk – 2-byte LE offset + 60 bytes of config_t data */
        if (report_id == REPORT_ID_DUALIE_CFG_CHUNK && bufsize >= 2) {
            uint16_t offset = (uint16_t)buffer[0] | ((uint16_t)buffer[1] << 8);
            uint16_t data_len = bufsize - 2;

            /* Guard against out-of-bounds writes */
            if ((uint32_t)offset + data_len > sizeof(global_state.hid_config_rx))
                return;

            memcpy(global_state.hid_config_rx + offset, buffer + 2, data_len);

            /* Track coverage: when we've received at least sizeof(config_t) bytes,
             * try to apply the config.  hid_config_rx_bytes is a high-water mark. */
            uint16_t end = offset + data_len;
            if (end > global_state.hid_config_rx_bytes)
                global_state.hid_config_rx_bytes = end;

            if (global_state.hid_config_rx_bytes >= sizeof(config_t))
                apply_hid_config(&global_state);
            return;
        }
    }

    /* ── Legacy deskhop vendor config protocol ───────────────────────────── */
