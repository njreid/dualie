...
// HID endpoint buffer size. Set to 64 (USB FS max packet size) so the vendor
// interface can receive 62-byte config chunks (report ID + 62 data bytes = 63).
// Keyboard/mouse reports are ≤ 10 bytes; the extra buffer is harmless.
