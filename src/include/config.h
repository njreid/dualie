...
/* key_remap_t.flags bits */
#define REMAP_FLAG_MOD_ONLY  0x01   /* modifier-only remap: transform modifier byte, no keycode required */

/* Daemon serialize.rs encodes config_t with this exact byte count.
 * If this assertion fails, update CONFIG_T_SIZE in daemon/src/serialize.rs. */
_Static_assert(sizeof(config_t) == 848,
    "config_t size changed – update CONFIG_T_SIZE in daemon/src/serialize.rs");
